...
public class ZarinpalBuy : MonoBehaviour
{
    ...
    IEnumerator RequestPayment(string callbackUrl, int pAmount)
    {
        ...
        using (UnityWebRequest req = UnityWebRequest.PostWwwForm("https://api.zarinpal.com/pg/v4/payment/request.json", ""))
        {
            ...
            else
            {
                ...
                StatusText.color = Color.red;
                StatusText.text = $"خطا در ارسال درخواست:{req.downloadHandler.text}";
                _Failed_panel.SetActive(true);
                _Paying_panel.SetActive(false);
            }
        }
